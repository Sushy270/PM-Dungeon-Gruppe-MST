...
public class PlayerSystem extends ECS_System {
    ...
    private void checkKeystroke(KSData ksd) {
        ...
        // check skills
        else if (Gdx.input.isKeyJustPressed(KeyboardConfig.FIRST_SKILL.get()))
            ksd.pc.getSkillSlot1().ifPresent(skill -> skill.execute(ksd.e));
        else if (Gdx.input.isKeyJustPressed(KeyboardConfig.SECOND_SKILL.get()))
            ksd.pc.getSkillSlot2().ifPresent(skill -> skill.execute(ksd.e));
        else if (Gdx.input.isKeyJustPressed(KeyboardConfig.THIRD_SKILL.get()))
